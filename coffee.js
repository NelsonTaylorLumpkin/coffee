...
for (const coffee of coffees) {
    if (coffee.includes("light")) {
        output += `I'll have the ${"light roast"} and drink it black`
    }
    else if (coffee.includes("medium")) {
        output += `I'll have the ${"mediumroast"} and add cream only`
    }
    else if (coffee.includes("dark")) {
        output += `I'll have the ${"Dark Roast"} and add cream and sugar`
    }
    output += "\n"
}
